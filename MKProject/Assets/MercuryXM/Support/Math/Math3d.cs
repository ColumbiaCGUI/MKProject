...
namespace MercuryXM.Support.Math
{
    public class Math3d {
        ...
        public static float CalcOptimalRotation(Transform from, Transform to, out Vector3 axis)
        {
            return CalcOptimalRotation(from.rotation, to.rotation, out axis);
        }

        public static float CalcOptimalRotation(Quaternion from, Quaternion to, out Vector3 axis)
        {
            var angle = Quaternion.Angle(from, to);

            if (System.Math.Abs(angle) < 0.0001f)
            {
                axis = Vector3.up;
                return 0;
            }

            var relativeRotation = Quaternion.Inverse(from) * to;

            relativeRotation.ToAngleAxis(out angle, out axis);

            if (angle <= 180.0f) return angle;
        ...
        {
            // angle in [0,180]
            var angleMagnitude = Vector3.Angle(a, b);
            var sign = Mathf.Sign(Vector3.Dot(n, Vector3.Cross(a, b)));

            // angle in [-179,180]
            var signedAngle = angleMagnitude * sign;
        ...
        public static float AngleVectorPlane(Vector3 vector, Vector3 normal){
            ...
            //this is in radians
            angle = (float)System.Math.Acos(dot);
